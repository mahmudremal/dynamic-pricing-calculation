...
class Columns {
	...
	public function manage_edit_product_columns($columns) {
		$columns['pops_actions'] = __('Pops', 'dpc');
		return $columns;
		$offset = 2;
		$pops_one = ['pops_actions' => __('Pops', 'dpc')];
	...
	public function manage_posts_custom_column($column, $post_id) {
		switch ($column) {
			case 'pops_actions':
				...
				$global_post_id = apply_filters('dpc/project/system/getoption', $global_key, 0);
				?>
				<button class="fwp-button fwppopspopup-open" type="button" <?php echo esc_attr(
					(
						apply_filters('dpc/project/system/isactive', 'standard-forceglobal') && 
						$global_post_id != get_the_ID()
					)?'disabled':''
				); ?>><?php esc_html_e('Open', 'dpc'); ?></button>
				<?php
				break;
			default:
				break;
	...
	public function post_row_actions($actions, $post) {
		...
		if($post->post_type =="product"){
			...
			$global_key = (isset($post_meta['product_type']) && $post_meta['product_type'] == 'sitting')?'sitting-global':'standing-global';
			$global_post_id = apply_filters('dpc/project/system/getoption', $global_key, 0);
			if(!(apply_filters('dpc/project/system/isactive', 'standard-forceglobal') && $global_post_id != get_the_ID())) {
				$config = ['id' => $post->ID];
				$actions['pops_actions'] = '<a href="#" class="fwppopspopup-open" type="button" data-config="' . esc_attr(json_encode($config)) . '">' . esc_html__('Setup', 'dpc') . '</a>';
			}
		}
		return $actions;
	}
	...
	public function manage_edit_shop_order_columns($columns) {
		$columns['teddy_attaches'] = __('Attached', 'dpc');
		return $columns;
	}
	public function manage_shop_order_custom_column($column, $post_id) {
		...
		switch($column) {
			case 'teddy_attaches':
				...
				$voiceDefination = ['tooltip' => __('Voice Skipped', 'dpc'), 'icon' => 'no-alt', 'defined' => 0];
				$order = wc_get_order($post_id);
				if($order && !is_wp_error($order)) {
					foreach($order->get_items() as $order_item_id => $order_item) {
						if($teddy_Voices->should_exists_voices($order, $order_item)) {
							if($voiceDefination['defined'] == 0) {
								$voiceDefination = ['tooltip' => __('Voice should be sent through email.', 'dpc'), 'icon' => 'email-alt', 'defined' => 1];
								// print_r("Should </br>");
							}
							if($teddy_Voices->has_single_voices($order, $order_item)) {
								if($voiceDefination['defined'] != 2) {
									$voiceDefination = ['tooltip' => __('Voice defined and is downloadable.', 'dpc'), 'icon' => 'download', 'defined' => 2];
									// print_r("Exists </br>");
								}
							}
						// } else {
		...
		if(isset($_POST['order_id'])) {
			...
			if($order && !is_wp_error($order)) {
				...
				if(count($json['attached']) <= 0) {
					$json['errorSVG'] = esc_url(DPC_BUILD_URI . '/icons/Information carousel_Monochromatic.svg');
				}
