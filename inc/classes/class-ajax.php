...
class Ajax {
	...
	public function search_product() {
		...
		$json = [
			...
			'product' => [
				...
				'positions' => [
					'standing' => apply_filters('dpc/project/system/getoption', 'standard-standingdoll', ''),
					'sitting' => apply_filters('dpc/project/system/getoption', 'standard-sittingdoll', ''),
				],
				'is_parent' => false,
		...
		if (!$json['product']['custom_fields'] || count($json['product']['custom_fields']) <= 0) {
			$json['product']['empty_image'] = DPC_BUILD_URI . '/icons/Team meeting_Monochromatic.svg';
		}
	...
	public function submit_popup() {
		...
		$json = [
			...
			'message' => __( 'Popup submitted successfully. Hold on unil you\'re redirecting to searh results.', 'dpc' )
		];
	...
	public function save_product() {
		$result = [];
		$result['hooks'] = ['product_updated'];$result['message'] = __( 'Popup updated Successfully', 'dpc' );
		$request = json_decode(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', stripslashes(html_entity_decode($_POST['dataset']))), true);
		$result['json'] = $request;
	...
	public function merge_customfields($fields) {
		...
		if(!is_user_logged_in()) {
			...
			$fields[] = [
				...
				'footer'	=> sprintf(__('By continuing, you confirm your agreement to our %sTerms & Conditions%s', 'dpc'), '<a href="'.esc_url(get_privacy_policy_url()).'" target="_blank">', '</a>'),
				'headerbgurl' => false,
				'extra_fields' => [
					[
						'fieldID' => ($fieldID+1),
	...
	public function update_orderitem() {
		...
		$json = ['hooks' => ['order_item_update_failed'], 'message' => __('Something went wrong. Please review your request again.', 'dpc')];
							...
							foreach($iRow as $j => $jRow) {
								if($field['steptitle'] == $jRow['title'] && $j == 0) {
									if(
										...
										$json['message'] = __('Successfully Updated your teddy bear name.', 'dpc');
	...
	public function suggested_names() {
		...
		$filteredKeys = array_keys(DPC_OPTIONS);
		$filteredData = [];
		foreach($filteredKeys as $key) {
			if(strpos($key, 'teddy-name-') !== false) {
				$filteredData[] = DPC_OPTIONS[$key];
			}
		}
		...
		if (apply_filters('dpc/project/system/isactive', 'names-randomize')) {
			shuffle($args['names']);
		}
	...
	public function checkout_fetch() {
		$json = ['hooks' => ['checkout-fetch-failed'], 'message' => __('Something went wrong. Please review your request again.', 'dpc')];
