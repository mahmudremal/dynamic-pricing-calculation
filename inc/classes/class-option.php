...
class Option {
  ...
  private $file;
	...
	protected function setup_hooks() {
		$this->file = DPC__FILE__;
		$this->plugin_slug = 'dpc';
		$this->plugin_name = 'dpc';
		$this->textdomain = 'dpc'; // str_replace('_', '-', $plugin_slug);
		// Initialise settings
		...
		add_filter('dpc/project/system/getoption', [$this, 'getOption'], 10, 2);
		add_filter('dpc/project/system/isactive', [$this, 'isActive'], 10, 1);
	}
	/**
	 * Initialise settings
	 * @return void
	...
	public function init() {
		$this->general = (object) wp_parse_args(apply_filters('dpc/project/settings/general', []), [
			'page_title'					=> __('Popup Configuration.', 'dpc'),
			'menu_title'					=> __('Customized Popup', 'dpc'),
			'role'							=> 'manage_options',
			'slug'							=> $this->plugin_slug,
			'page_header'					=> __('Teddy bear customization popup.', 'dpc'),
			'page_subheader'				=> __('Customization popup configuration screen. Default values and truning on/off can be done from here.', 'dpc'),
			'no_password'					=> __('A password is required.', 'dpc'),
		]);
	}
	...
	public function add_settings_link($links) {
		$settings_link = '<a href="options-general.php?page='.$this->general->slug.'">' . __('Settings', 'dpc') . '</a>';
		// array_push($links, $settings_link);
		$links[] = $settings_link;
		$links[] = '<a href="https://www.fiverr.com/mahmud_remal/" target="_blank">' . __('Developer Support', 'dpc') . '</a>';
		return $links;
	}
	/**
	...
	private function settings_fields() {
		$settings = [];
		$settings = apply_filters('dpc/project/settings/fields', $settings);
		return $settings;
	}
	/**
	 * Options getter
	...
	public function display_field($args) {
		...
		switch($field['type']) {
			...
			case 'radio':
			case 'select_multi':
				$html .= apply_filters('dpc/project/settings/fields/label', '<br/><span class="description">' . $field['description'] . '</span>', $field);
			break;
			default:
				$html .= apply_filters('dpc/project/settings/fields/label', '<label for="' . esc_attr($field['id']) . '"><span class="description">' . $field['description'] . '</span></label>' . "\n", $field);
			break;
		}
		echo $html;
	}
	...
	public function getOption($option, $default) {
		return isset(DPC_OPTIONS[$option])?DPC_OPTIONS[$option]:$default;
	}
	/**
	...
	public function isActive($option) {
		return (isset(DPC_OPTIONS[$option]) && DPC_OPTIONS[$option] == 'on');
	}
}