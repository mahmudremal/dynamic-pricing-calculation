...
class I18n {
	...
	public function load_plugin_textdomain() {
		load_plugin_textdomain('dpc', false, dirname(plugin_basename(DPC__FILE__)) . '/languages');
	}
	public function js_translates() {
		$translates = [
			'submit' => __('Submit', 'dpc'),
			// ...
		];
	...
	public function get_locale($default = false) {
		return apply_filters('dpc/project/system/getoption', 'translate-toonly', 'user') == 'user'?get_user_locale():get_locale();
	}
	/**
	...
	public function translate_string($text, $domain, $name, $language_code = NULL) {
		if ($language_code === NULL) {
			$language_code = apply_filters('dpc/project/system/getoption', 'translate-toonly', 'user') == 'user'?get_user_locale():get_locale();
		}
	...
	public function translate_number($number, $decimal = 0, $locale = NULL) {
		if (!$locale || empty($locale)) {
			$locale = apply_filters('dpc/project/system/get_locale', get_user_locale());
		}
	...
	public function lectoTranslateText($texts, $to, $from) {
		$apiKey = apply_filters('dpc/project/system/getoption', 'translate-api', 'V13Y91F-DR14RP6-KP4EAF9-S44K7SX');
	...
	public function libreTranslateText($text, $translate_to, $translate_from, $language_code) {
		$apiKey = apply_filters('dpc/project/system/getoption', 'translate-api', '');
	...
	public function get_translations($key, $def = []) {
		$filename = DPC_DIR_PATH . '/templates/' . $key.'.txt';
